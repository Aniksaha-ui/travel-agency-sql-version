...
const insertIntoBookingPerson = async (person,bookingId,tourId,userId) => {
  ...
  console.log('Inserted IDs:', insertedIds);
    ...
    const fetchUserTourDetails = async(userId,tourId,bookingId)=>{
      ...
      const personInformation = await db.sequelize.query(`SELECT name,address FROM booking_persons WHERE userId=${userId} AND tourId=${tourId} AND bookingId=${bookingId}`
      ,{ type: QueryTypes.SELECT });
      
      const data = {tour:tourInformation,hotel:hotelInformation,persons:personInformation}
      return data;
    }
